...
class CMUScraper(BaseScraper):
    ...
    API_URL = "https://cmu.flintbox.com/api/v1/technologies"
    ORGANIZATION_ID = "18"
    ...
    @property
    def name(self) -> str:
        return "Carnegie Mellon CTTEC"

    async def _init_session(self) -> None:
        """Initialize aiohttp session."""
        if self._session is None:
            self._session = aiohttp.ClientSession()
            logger.debug("HTTP session initialized for CMU")
        ...
        try:
            async with self._session.get(self.API_URL, params=params) as response:
                if response.status == 200:
                    ...
                    logger.debug(f"CMU has {total_pages} pages")
                    return total_pages
                else:
    ...
    async def scrape(self) -> AsyncIterator[Technology]:
        """Scrape all technologies from CMU Flintbox."""
        try:
            await self._init_session()
