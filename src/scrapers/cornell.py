...
class CornellScraper(BaseScraper):
    ...
    API_URL = "https://cornell.flintbox.com/api/v1/technologies"
    ORGANIZATION_ID = "25"
    ...
    @property
    def name(self) -> str:
        return "Cornell Center for Technology Licensing"

    async def _init_session(self) -> None:
        """Initialize aiohttp session."""
        if self._session is None:
            self._session = aiohttp.ClientSession()
            logger.debug("HTTP session initialized for Cornell")
        ...
        try:
            async with self._session.get(self.API_URL, params=params) as response:
                if response.status == 200:
                    ...
                    meta = data.get("meta", {})
                    total_pages = meta.get("totalPages", 47)
                    logger.debug(f"Cornell has {total_pages} pages")
                    return total_pages
                else:
    ...
    async def scrape(self) -> AsyncIterator[Technology]:
        """Scrape all technologies from Cornell Flintbox."""
        try:
            await self._init_session()
    ...
    async def _parse_api_item_with_detail(self, item: dict) -> Optional[Technology]:
        ...
        try:
            ...
            description = None
            if detail:
                # Try to get fuller description from detail
