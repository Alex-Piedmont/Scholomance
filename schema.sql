...
-- Coverage items: weekly independent-coverage finds from the commercialization
-- pipeline. These are NOT TTO listings. Optional technology_uuid joins to
-- technologies.uuid when a best-effort match exists; unmatched items stay NULL.
-- Pipeline workflow (hold/proceed/archive) hangs off coverage_items, never off
-- scraped listing rows. Tables already exist in production (2026-08-25);
-- statements are IF NOT EXISTS so re-apply is a no-op.
...
COMMENT ON COLUMN coverage_items.technology_uuid IS 'Optional best-effort join to technologies.uuid; unmatched items stay NULL. Not a foreign key.';
COMMENT ON COLUMN coverage_items.packet_week IS 'Monday of the briefing week.';
